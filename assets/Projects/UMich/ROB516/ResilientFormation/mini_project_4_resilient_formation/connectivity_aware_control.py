...
# Video settings
fps = 20
metadata = dict(title='connectivity_aware Simulation', artist='Matplotlib', comment='Multi-robot CBF simulation')
# writer = FFMpegWriter(fps=fps, metadata=metadata)
writer = PillowWriter(fps=fps, metadata=metadata)
video_name = "video_4_4a.gif"


# CBF alphas - tuned for connectivity-aware control but can be adjusted 
alpha_conn = 3.0
alpha_coll = 2.5
alpha_obs = 5.0
...
with writer.saving(fig, video_name, dpi=100):
    for step in range(num_steps):
        ...
        # Connectivity CBF
        h_conn = connectivity_CBF(L, epsilon)
        h_conn_history.append(h_conn)
        time_steps.append(step * dt)
        print("time step, h_conn:", step, h_conn)
        ...
        constraints.append(cp.sum(mu_m(robots, u, R, sigma)) >= -alpha_conn * h_conn)
...
plotter(time_steps, h_conn_history, epsilon, 'Connectivity-Aware Control: h Function Evolution')
